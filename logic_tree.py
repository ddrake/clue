...
class tree:
    ...
    def add_pos(self, query):
        """ add a query (tuple) positively to the tree
            intentionally creating duplicate references to atoms
        """
        if self.branches:
            self.branches = [b + [atom(q,True)] for b in self.branches for q in query]
        else:
            for q in query: self.branches.append([atom(q,True)])
        self.prune()
        self.clean()
    ...
    def possibles(self):
        ...
        lst = [sorted(list(b-isect),key=lambda a: a.num) for b in lst if len(b-isect) > 0]
        return [[l.num for l in sub] for sub in lst]
        
    def simple(self):
        """ return a simple representation of the tree """
        if not self.branches: return self.branches
        lst = [set(b) for b in self.branches]
        isect = self.common_elements()
        difflst = [sorted(list(b-isect),key=lambda a: a.num) for b in lst if len(b-isect) > 0]
    ...
    def clean(self):
        ...
        """
        # remove duplicates from each branch, and then remove any duplicate sets
        ...
        for i in range(lenlst):
            ...
            for j in range(lenlst):
                ...
                if lst[i].issubset(lst[j]):
                    exclude = True
                    break
            if not exclude: newlst.append(lst[i])
